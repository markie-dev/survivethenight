...
{ 
  ...
  if (t == eSprite::Battery || t == eSprite::LogicBoard || t == eSprite::Antenna)
      m_bIsRadio = true;
...
void CObject::setSpriteType(eSprite es) {
    spriteType = es;
}
...
void CObject::ActiveCollisionResponse(const Vector2& norm, float d, CObject* pObj) {
    if (m_bDead)return; //dead, bail out
} //CollisionResponse
...
const bool CObject::isActivity() const {
  return m_bIsActivity;
} //isActivity

const bool CObject::isRadio() const {
    return m_bIsRadio;
} //isActivity